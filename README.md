...
1. O CLI le o JSON e valida a estrutura basica.
2. Cada frase recebe um GUID estavel a partir do texto em frances.
3. O pipeline escolhe duas vozes `fr-FR` de forma deterministica por frase.
4. O `edge-tts` gera duas locucoes: uma normal e outra um pouco mais lenta.
5. O `pydub` concatena `Audio1 + 2s de pausa + Audio2` em um unico `.mp3`.
6. O `genanki` cria 1 nota por frase e 2 cartas por nota, com midia embutida no `.apkg`.
7. Depois da geracao do `.apkg`, a aplicacao tenta sincronizar com o Anki via AnkiConnect.
8. Se o AnkiConnect nao estiver disponivel, a execucao termina com sucesso e apenas registra um aviso.
